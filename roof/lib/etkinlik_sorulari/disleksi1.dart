...
class _WordMatchingScreenState extends State<WordMatchingScreen> with SingleTickerProviderStateMixin {
  final List<String> firstSyllables = ['par', 'ki', 'dok', 'gö', 'men', 'dü', 'ör', 'bon'];
  final List<String> secondSyllables = ['mak', 'tap', 'tor', 'bek', 'dil', 'dük', 'dek', 'cuk'];
  ...
  void initState() {
    ...
    _animation = Tween<double>(begin: 0, end: 2 * pi * 5).animate(CurvedAnimation(
      parent: _controller,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < syllables.length; i++) {
      ...
      canvas.drawArc(
        ...
        sweepAngle,
        true,
        paint..color = i % 2 == 0 ? const Color.fromARGB(255, 134, 241, 182) : const Color.fromARGB(255, 219, 130, 207),
      );
      ...
      textPainter.layout();
      final offset = Offset(
        center.dx + radius / 2 * cos(startAngle + sweepAngle / 2) - textPainter.width / 2,
        center.dy + radius / 2 * sin(startAngle + sweepAngle / 2) - textPainter.height / 2,
      );
...
class SyllableBox extends StatelessWidget {
  ...
  final bool used;
  SyllableBox(this.syllable, {this.used = false});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        syllable,
        style: TextStyle(fontSize: 20, color: used ? Colors.green : Colors.black),
      ),
    );
  }
}
